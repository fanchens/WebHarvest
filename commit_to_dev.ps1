# Git 提交脚本 - 提交到 dev 分支
# 仓库地址: https://gitee.com/fanchenn/web-harvest.git

$ErrorActionPreference = "Stop"

# 配置信息
$REPO_URL = "https://gitee.com/fanchenn/web-harvest.git"
$BRANCH = "dev"
...
if (-not $remoteExists) {
    ...
    if ($currentUrl -ne $REPO_URL) {
        ...
        Write-Host "✓ 远程仓库地址已更新: $REPO_URL" -ForegroundColor Green
    } else {
        Write-Host "✓ 远程仓库已配置: $REPO_URL" -ForegroundColor Green
    }
}
...
if ([string]::IsNullOrEmpty($currentBranch)) {
    ...
    Write-Host "✓ 已创建并切换到 dev 分支" -ForegroundColor Green
    ...
    Write-Host "✓ 已切换到 dev 分支" -ForegroundColor Green
} else {
    Write-Host "✓ 当前已在 dev 分支" -ForegroundColor Green
}
